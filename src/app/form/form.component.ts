...
export class FormComponent implements OnInit {
  ...
  get nome() { return this.bioForm.get('nome'); }
  get profissao() { return this.bioForm.get('profissao'); }
  get estilo() { return this.bioForm.get('estilo'); }
  get comprimento() { return this.bioForm.get('comprimento'); }
  get opcoes() { return this.bioForm.get('opcoes'); }
  ...
  generateBio(): void {
    ...
    // Chama o serviço para gerar as bios
    this.bioService.generateBio(
      this.nome?.value,
    ...
    .subscribe({
      ...
      if (response && response.bios) {
        this.biosGenerated.emit(response.bios.message.content[0].text.split('\n').filter((bio: string) => bio.trim() !== ''));
      ...
      error: (error) => {
      this.snackBar.open('Erro ao se comunicar com a IA. Tente novamente mais tarde.', 'Fechar', {
        duration: 5000
      });
